...
from maix import image, camera, display, time, nn, touchscreen, rtsp, audio, app
...
class Target:
    ...
    def __init__(self, out_range:float, ignore_limit:float, path:str):
        ...
        # Create higher resolution camera for RTSP streaming
        self.rtsp_cam = camera.Camera(2560, 1440, image.Format.FMT_YVU420SP)
        # Create RTSP server and bind camera
        self.server = rtsp.Rtsp()
        self.server.bind_camera(self.rtsp_cam)
        self.server.start()
        print(f"RTSP Server URL: {self.server.get_url()}")
        
        # Create a camera with the resolution required by the detector
...
if __name__ == '__main__':
    ...
    try:
        ...
        while wait_time_s:
            ...
            time.sleep(1)
            wait_time_s -= 1
        target.cleanup()  # Clean up RTSP server on error
        exit(-1)
    ...
    try:
        ...
        # Ensure RTSP server is properly stopped when the program exits
        target.cleanup()